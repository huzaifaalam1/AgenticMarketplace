...
export async function POST(req: NextRequest) {
  try {
    console.log('DISPUTE VALIDATION API HIT')

    const { contract, events, dispute } = await req.json()

    const prompt = `
You are a strict JSON generator validating a dispute claim.
...
{
  "is_valid": boolean (true if dispute is valid based on contract and evidence),
  "reasoning": string (brief explanation of why the dispute is valid or not),
...
- Evaluate the dispute claim against the contract terms and evidence
- Determine if the dispute is legitimate based on the contract and available evidence
...
`
    ...
    const callModel = async () => {
      const res = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent?key=${process.env.GEMINI_STAGE5_KEY}`,
        {
          method: 'POST',
